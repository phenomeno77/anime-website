<template>
  <div
    class="container mx-auto px-4 py-12 max-w-3xl leading-relaxed text-slate-200"
  >
    <h1 class="text-3xl font-bold mb-6 text-[var(--manga-gold)]">
      Terms of Use
    </h1>

    <p class="text-sm text-slate-400 mb-8">
      By accessing and using this website, you agree to the following terms and
      conditions.
    </p>

    <div
      class="space-y-6 bg-[#15151A] p-6 rounded-xl border border-[#27272A] shadow-lg"
    >
      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Copyright
        </h2>

        <p>
          All artwork, illustrations, graphics, photographs, designs, texts,
          logos and other content published on this website are protected by
          Austrian and international copyright laws.
        </p>

        <p class="mt-2">
          Unless otherwise stated, all intellectual property rights belong to
          AnimeArt Marianelo.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Permitted Use
        </h2>

        <p>
          You may browse this website for personal, non-commercial purposes
          only.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Prohibited Use
        </h2>

        <p>Without prior written permission, you may not:</p>

        <ul class="list-disc pl-6 mt-3 space-y-2 text-slate-300">
          <li>Copy or download artwork.</li>
          <li>Redistribute images on websites or social media.</li>
          <li>Sell, license or commercially exploit any artwork.</li>
          <li>Edit, trace, modify or create derivative works.</li>
          <li>Use artwork in printed or digital publications.</li>
          <li>Use artwork for NFTs or blockchain projects.</li>
          <li>Use artwork in advertising or marketing.</li>
          <li>Claim ownership or authorship.</li>
        </ul>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Artificial Intelligence
        </h2>

        <p>
          The artwork displayed on this website may not be used for training,
          fine-tuning, developing or evaluating artificial intelligence, machine
          learning models, image generators or similar technologies without
          explicit written permission.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Commissions
        </h2>

        <p>
          Purchasing artwork or commissioning illustrations does not transfer
          copyright unless explicitly agreed in writing. Any granted license is
          limited to the agreed purpose.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          External Links
        </h2>

        <p>
          This website may contain links to third-party websites. The operator
          accepts no responsibility for the content or privacy practices of
          external websites.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Disclaimer
        </h2>

        <p>
          Although every effort has been made to ensure that the information on
          this website is accurate and up to date, no guarantee is given
          regarding completeness, accuracy or availability.
        </p>
      </div>

      <div>
        <h2 class="text-lg font-semibold text-[var(--manga-accent)] mb-2">
          Governing Law
        </h2>

        <p>
          These Terms of Use are governed by the laws of the Republic of
          Austria. Where legally permissible, the competent court at the
          operator's place of business shall have jurisdiction.
        </p>
      </div>
    </div>
  </div>
</template>
